...
if __name__ == "__main__":
    print()
    print("=" * 60)
    print("  Semantic Cache Proxy - Phase 4")
    print("=" * 60)
    print(f"  Server:     http://localhost:{config.proxy_port}")
    print(f"  OpenAI URL: http://localhost:{config.proxy_port}/v1")
    print(f"  Dashboard:  http://localhost:{config.proxy_port}/v1/dashboard")
    print(f"  Metrics:    http://localhost:{config.proxy_port}/metrics")
    print(f"  Providers:  {config.default_provider}")
    print(f"  Threshold:  {config.similarity_threshold}")
    print(f"  Adaptive:   {config.enable_adaptive_threshold}")
    print("=" * 60)
    print()
